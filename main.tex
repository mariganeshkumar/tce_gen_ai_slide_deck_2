...
\item Solution: Variational approximation with $q(z|x)$ (approximate posterior for $p(z|x)$)
\end{itemize}
...
\begin{frame}[fragile]{Mathematics of Reparameterization}
\begin{itemize}
...
\centering
\begin{lstlisting}
def reconstruction_loss(x, x_params, distribution):
    if distribution == 'bernoulli':
        return F.binary_cross_entropy(x_params, x, reduction='sum')
    elif distribution == 'gaussian':
        return F.mse_loss(x_params, x, reduction='sum')
\end{lstlisting}
\end{frame}
...
def decode(z, c):
    ...
    hidden = self.decoder_net(combined)
