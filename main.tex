...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Introduction to Autoencoders \& VAEs}
...
\begin{itemize}
\item VAE is a latent variable model: $p(x,z) = p(z)p(x|z)$
...
% Integration
\node[draw,rounded corners,text width=9cm] at (3.5,-2) {$p(x) = \int p(z)p(x|z)dz$ (intractable)\\
$\log p(x) \geq \mathbb{E}_{q(z|x)}[\log p(x|z)] - D_{KL}(q(z|x) \| p(z))$ (ELBO)};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{frame}{Parameterizing $q(z|x)$ as Gaussian}
  \begin{columns}
    ...
    Encoder outputs:
        \begin{itemize}
          \item Mean vector: $\mu_\phi(x) \in \mathbb{R}^d$
          \item Log-variance: $\log \sigma^2_\phi(x) \in \mathbb{R}^d$
        \end{itemize}
  \end{columns}
\end{frame}
...
\begin{itemize}
\item $\mu(x) \in \mathbb{R}^d$ defines center of $q(z|x)$
...
\begin{itemize}
\item $\sigma^2(x) \in \mathbb{R}^d$ defines uncertainty in $q(z|x)$
...
\draw[->,thick] (0,-2) -- (8,-2) node[right] {$z$};
\draw[->,thick] (0,-2) -- (0,0) node[above] {$q(z|x)$};

\draw[domain=0:8, smooth, variable=\x, red, thick] 
  plot ({\x}, {-2+1.5*exp(-(\x-4)^2/0.5)});
\draw[domain=0:8, smooth, variable=\x, blue, thick] 
  plot ({\x}, {-2+1.5*exp(-(\x-4)^2/2.0)});

\node[red] at (2.5,-1) {Low variance};
\node[blue] at (6.5,-1) {High variance};
\end{tikzpicture}
\end{center}
\end{frame}
...
\item Input: latent vector $z \in \mathbb{R}^d$
...
\draw[<->,thick,dashed] (x1) to[bend below] node[below] {reconstruction error} (x2);
\end{tikzpicture}
...
\begin{itemize}
\item Goal: Maximize conditional log-likelihood $\log p(x|c)$
\item Derivation parallels standard ELBO:
\begin{align}
\log p(x|c) &\geq \mathbb{E}_{q(z|x,c)}[\log p(x|z,c)] - D_{KL}(q(z|x,c) \| p(z|c))
\end{align}
\item Components:
  \begin{itemize}
  \item Conditional reconstruction: $\mathbb{E}_{q(z|x,c)}[\log p(x|z,c)]$
  \item Conditional KL divergence: $D_{KL}(q(z|x,c) \| p(z|c))$
  \end{itemize}
\end{itemize}
...
\node[rectangle,draw,fill=green!20,rounded corners,minimum width=3.5cm,minimum height=1cm] (rec) at (-2.5,-2) {Conditional\\Reconstruction};
\node[rectangle,draw,fill=red!20,rounded corners,minimum width=3.5cm,minimum height=1cm] (kl) at (2.5,-2) {Conditional\\KL Divergence};
